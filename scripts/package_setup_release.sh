...
#
# Writes: dist/monster-rancher-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix monster-rancher \
  --exe-name Monster_Rancher_Recompiled \
  --display-name "Monster Rancher Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --runtime-dir mods \
  --disc-hint "Monster Rancher (USA).cue" \
